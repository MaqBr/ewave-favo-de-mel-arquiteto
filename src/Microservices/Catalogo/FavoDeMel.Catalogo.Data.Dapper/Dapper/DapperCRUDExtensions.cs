...
namespace Data.Dapper
{
    ...
    /// </summary>
    public static partial class DapperCRUDExtensions
    {
        static DapperCRUDExtensions()
        {
            SetDialect(_dialect);
        }
        ...
        public static T Get<T>(this IDbConnection connection, int id, IDbTransaction transaction = null, int? commandTimeout = null)
        {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Returns a single entity by a single id from table T.</returns>
        public static T Get<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Returns a single entity by a single id from table T.</returns>
        public static T GetProperties<T>(this IDbConnection connection, object id, object properties, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            ...
            foreach (var prop in properties.GetType().GetProperties())
            {
                ...
                if (ShouldSelectProperty(currenttype, propertyName) == false)
                    continue;

                if (sbProperties.Length > 0)
                    sbProperties.AppendFormat(", ");

                sbProperties.AppendFormat("{0} as {1}", GetColumnName(prop), propertyName);
            }
        ...
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, IDbTransaction transaction)
        {
            return connection.GetList<T>(new { }, transaction);
        }
        ...
        public static int UpdateList<TEntity>(this IDbConnection connection, object propertiesWithValue, object whereConditions,
            IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (propertiesWithValue == null)
                throw new ArgumentException("Entity's properties must be informed.");
            ...
            var sbProperties = new StringBuilder();
            foreach (var prop in propertiesWithValue.GetType().GetProperties())
            {
                var propertyName = prop.Name;

                //Checks if this property is allowed to be updated.
                if (ShouldUpdateProperty(entityType, propertyName) == false)
                    continue;

                var propertyValue = prop.GetValue(propertiesWithValue);

                if (sbProperties.Length > 0)
                    sbProperties.AppendFormat(", ");

                sbProperties.AppendFormat("{0} = @{1}", propertyName, propertyName);
                entityInstance.SetPropertyValue(propertyName, propertyValue);
            }
            ...
            if (whereprops.Any())
            {
                ...
                {
                    if (GetScaffoldableProperties(entityType).ToArray().Any(a => a.Name == prop.Name) == false)
                    {
                        throw new Exception($"Propriedade '{prop.Name}' não encontrada no objeto informado do tipo {entityType}");
                    }

                    var propertyValue = prop.GetValue(whereConditions);
                    entityInstance.SetPropertyValue(prop.Name, propertyValue);
                }

                sb.Append(" where ");

                BuildWhere(whereConditions.GetType(), sb, whereprops, whereConditions);
            }

            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("UpdateList<{0}> {1}", entityType, sb));

            return connection.Execute(sb.ToString(), entityInstance, transaction, commandTimeout);
        }
		...
		/// </summary>
		/// <param name="connection"></param>
		/// <param name="propertiesWithValue"></param>
		/// <param name="transaction"></param>
		/// <param name="commandTimeout"></param>
		/// <returns>The number of affected records</returns>
		public static int UpdateProperties<TEntity>(this IDbConnection connection,
                                                    TEntity entityInstance,
                                                    object propertiesWithValue,
                                                    IDbTransaction transaction = null,
                                                    int? commandTimeout = null)
        {
            var idProps = GetIdProperties(entityInstance).ToList();
            ...
            var name = GetTableName(entityInstance);
            ...
            foreach (var prop in propertiesWithValue.GetType().GetProperties())
            {
                ...
                //Checks if this property is allowed to be updated.
                if (ShouldUpdateProperty(entityInstance, propertyName) == false)
                    continue;
            ...
            sb.Append(" where ");
            BuildWhere<TEntity>(sb, idProps, entityInstance);
        ...
        public static int DeleteList<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            {
                sb.Append(" where ");
                BuildWhere(entityType, sb, whereprops, whereConditions);
            ...
            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("DeleteList<{0}> {1}", entityType, sb));
        ...
        public static int RecordCount<TEntity>(this IDbConnection connection, string conditions = "", object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Returns a count of records.</returns>
        public static int RecordCount<TEntity>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(TEntity);
            var name = GetTableName(currenttype);
        ...
        private static void BuildWhere(Type objectType, StringBuilder sb, IEnumerable<PropertyInfo> idProps, object whereConditions = null)
        {
            var propertyInfos = idProps.ToArray();
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                //by converting the model type to the database column name via the column attribute
                var propertyToUse = propertyInfos.ElementAt(i);
                var sourceProperties = GetScaffoldableProperties(objectType).ToArray();
                for (var x = 0; x < sourceProperties.Count(); x++)
                {
                    if (sourceProperties.ElementAt(x).Name == propertyToUse.Name)
                    {
                        if (whereConditions != null && propertyToUse.CanRead)
                        {
                            var valorPropriedade = whereConditions.GetType().GetProperty(propertyToUse.Name).GetValue(whereConditions, null);
                            if (valorPropriedade == null || valorPropriedade == DBNull.Value)
                            {
                                useIsNull = true;
                            }
        ...
        {
            return GetScaffoldableProperties(typeof(T));
        }

        private static IEnumerable<PropertyInfo> GetScaffoldableProperties(object instancia)
        {
            return GetScaffoldableProperties(instancia.GetType());
        }

        //Get all properties that are not decorated with the Editable(false) attribute
        private static IEnumerable<PropertyInfo> GetScaffoldableProperties(Type entityType)
        {
        ...
        /// <summary>
        /// Retorn if a property should be updated.
        /// </summary>
        /// <typeparam name="T">Entity type</typeparam>
        /// <param name="entity">Entity to update.</param>
        /// <param name="propertyName">Name of the property being checked.</param>
        /// <returns></returns>
        private static bool ShouldUpdateProperty<T>(T entity, string propertyName)
        {
            var updateableProperties = GetScaffoldableProperties<T>().Where(p => p.Name == propertyName);

            var restricted = updateableProperties.Any(p => p.Name.Equals("Id", StringComparison.OrdinalIgnoreCase) == false &&
                                                            p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(KeyAttribute).Name) == false &&
                                                            p.GetCustomAttributes(true).Any(attr => (attr.GetType().Name == typeof(ReadOnlyAttribute).Name) && IsReadOnly(p)) &&
                                                            p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(IgnoreUpdateAttribute).Name) == false &&
                                                            p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(NotMappedAttribute).Name) == false);
            return (restricted == false);
        }

        private static bool ShouldSelectProperty(Type entityType, string propertyName)
        {
            var property = GetScaffoldableProperties(entityType).FirstOrDefault(p => p.Name == propertyName);
